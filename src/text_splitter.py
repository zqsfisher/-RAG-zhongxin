...
class TextSplitter():
    ...
    def split_markdown_reports(self, all_md_dir: Path, output_dir: Path, chunk_size: int = 30, chunk_overlap: int = 5, subset_csv: Path = None):
        ...
        for md_path in all_md_paths:
            ...
            # 如果 sha1 为空，使用 MD5 哈希作为替代
            if not sha1:
                sha1 = hashlib.md5(file_no_ext.encode('utf-8')).hexdigest()
                print(f"警告：{md_path.name} 在 subset.csv 中未找到，使用 MD5 哈希 '{sha1}' 作为 sha1")
                # 记录新的映射关系
                new_records.append({
                    'sha1': sha1,
        ...
        # 将新记录追加写入 subset.csv
        if new_records and subset_csv is not None:
            new_df = pd.DataFrame(new_records)
            # 追加写入，使用 gbk 编码保持一致
            new_df.to_csv(subset_csv, mode='a', header=False, encoding='gbk', index=False)
            print(f"已将 {len(new_records)} 条新记录追加写入 {subset_csv}")
