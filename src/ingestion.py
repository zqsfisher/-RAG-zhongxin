...
class VectorDBIngestor:
    ...
    def process_reports(self, all_reports_dir: Path, output_dir: Path):
        ...
        import os
        all_report_paths = list(all_reports_dir.glob("*.json"))
        # 使用 os.makedirs 确保输出目录存在，兼容中文路径
        ...
        for report_path in tqdm(all_report_paths, desc="Processing reports for FAISS"):
            ...
            if not sha1:
                # 如果 sha1 为空，使用 md5(file_name) 生成纯英文数字的标识符
                import hashlib
                file_name = report_data["metainfo"].get("file_name", report_path.stem)
                sha1 = hashlib.md5(file_name.encode('utf-8')).hexdigest()
                print(f"使用 MD5 哈希 '{sha1}' 作为替代")

            # 序列化后由 Python open 写入，绕过 FAISS 文件 I/O（FAISS 不支持含中文的路径）
