...
class DocumentSummaryPrompt:
    instruction = """
你是一个金融文档分析专家。
你的任务是阅读一份中文金融研究报告或公司年报的开头部分，生成一个200字以内的结构化摘要。

摘要必须包含以下信息：
1. 文档类型（例如：券商研报、年报、调研纪要）
2. 发布机构
3. 覆盖的时间范围
4. 核心主题和关键观点（2-3个要点）
5. 提到的关键财务指标名称（如营收、毛利率、产能利用率等）
6. 如果有投资评级，请注明

请用中文回答。摘要应简洁但信息丰富，足以用于后续的文档检索路由。
"""

    class SummarySchema(BaseModel):
        summary: str = Field(description="200字以内的综合摘要")
        key_topics: List[str] = Field(description="2-5个核心主题或关键词")
        time_period: str = Field(description="文档覆盖的时间范围，如'2024Q4'、'2024年全年'")
        document_type: str = Field(description="文档类型：券商研报/年报/调研纪要/其他")
        issuing_institution: str = Field(description="发布机构名称")
        investment_rating: str = Field(default="无", description="投资评级，如买入/增持/中性/无")

    pydantic_schema = re.sub(r"^ {4}", "", inspect.getsource(SummarySchema), flags=re.MULTILINE)

    user_prompt = "以下是文档开头部分的文本：\n\"\"\"\n{document_text}\n\"\"\"\n\n请生成结构化摘要。"
    system_prompt = build_system_prompt(instruction, pydantic_schema=pydantic_schema)


class DocumentRoutingPrompt:
    instruction = """
你是一个文档检索路由专家。
你手头有多份金融文档的摘要。用户会提出一个问题，你需要判断哪些文档最可能包含回答该问题所需的信息。

要求：
1. 仔细阅读每个文档的摘要
2. 判断问题涉及的指标、时间段、主题是否与每个文档相关
3. 选择最多{max_documents}个最相关的文档
4. 如果问题涉及特定时间段的数据，优先选择覆盖该时间段的文档
5. 如果问题涉及特定指标，优先选择摘要中提到该指标的文档
6. 输出所选文档的索引（从1开始），按相关性从高到低排序
"""

    class RouteSelection(BaseModel):
        selected_indices: List[int] = Field(description="按相关性排序的文档索引列表（1-based），最多不超过规定数量")
        reasoning: str = Field(description="选择理由，简要说明为何选择这些文档")

    pydantic_schema = re.sub(r"^ {4}", "", inspect.getsource(RouteSelection), flags=re.MULTILINE)
