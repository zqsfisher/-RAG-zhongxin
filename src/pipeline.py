...
class Pipeline:
    ...
    def generate_document_summaries(self):
        """为所有已分块报告生成摘要，保存到 databases/document_summaries.json"""
        from src.document_summarizer import DocumentSummarizer

        output_path = self.paths.databases_path / "document_summaries.json"
        summarizer = DocumentSummarizer(
            documents_dir=self.paths.documents_dir,
            api_provider=self.run_config.api_provider,
            model=self.run_config.answering_model,
        )
        result = summarizer.generate_all_summaries(output_path)
        count = len(result.get("documents", {}))
        print(f"Document summaries saved to {output_path} ({count} documents)")
    ...
    def process_questions(self):
        # 处理所有问题，生成答案文件
        summaries_path = self.paths.databases_path / "document_summaries.json" if self.run_config.use_document_routing else None
        processor = QuestionsProcessor(
            ...
            use_reranking=self.run_config.use_reranking,
            reranking_sample_size=self.run_config.reranking_sample_size,
            top_n_retrieval=self.run_config.top_n_retrieval,
            parallel_requests=self.run_config.parallel_requests,
            api_provider=self.run_config.api_provider,
            answering_model=self.run_config.answering_model,
            full_context=self.run_config.full_context,
            use_document_routing=self.run_config.use_document_routing,
    ...
    def answer_single_question(self, question: str, kind: str = "string", with_trace: bool = False):
        """
        单条问题即时推理，返回结构化答案（dict）。
        kind: 支持 'string'、'number'、'boolean'、'names' 等
        with_trace: 是否返回完整 RAG 流程追踪数据
        """
        t0 = time.time()
        if with_trace:
            print("[计时] 开始初始化 QuestionsProcessor (trace mode) ...")
        summaries_path = self.paths.databases_path / "document_summaries.json" if self.run_config.use_document_routing else None
        processor = QuestionsProcessor(
            vector_db_dir=self.paths.vector_db_dir,
            documents_dir=self.paths.documents_dir,
...
max_config = RunConfig(
    ...
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + Jina reranking + SO CoT; llm = qwen-turbo",
    answering_model="qwen-turbo-latest",
    config_suffix="_qwen_turbo"
)


routing_config = RunConfig(
    use_document_routing=True,
    parent_document_retrieval=True,
    use_reranking=True,
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Document Routing + Parent Document Retrieval + Jina reranking + SO CoT; llm = qwen-turbo",
    answering_model="qwen-turbo-latest",
    config_suffix="_routing"
)
...
if __name__ == "__main__":
    ...
    # 6.5 为所有文档生成摘要，保存到 databases/document_summaries.json
    print('6.5 为所有文档生成摘要，保存到 databases/document_summaries.json')
    pipeline.generate_document_summaries()
